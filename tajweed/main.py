...
from TajweedAudioSegmenter import TajweedAudioSegmenter
from quran_tajweed.tajweed_classifier import *
import torch
import matplotlib
matplotlib.use('Agg')
...
if "asr_model" not in globals():
    ...
    # Load tajweed rule trees
    rule_trees = load_rule_trees()

# # Define audio file path
# audio_file_path = r"D:\Ai\gradeation projrct\Quran Memorization\wav_audio_files\husary\surah_1\001007.wav"

# # Process audio with tajweed classification
# result = process_audio_with_tajweed(audio_file_path, asr_model, rule_trees)

# # Visualize the results
# if result:
#     visualize_tajweed_results(result)

#     # Save the results to a JSON file
#     output_file = "tajweed_with_timestamps.json"
#     with open(output_file, "w", encoding="utf-8") as f:
#         json.dump(result, f, ensure_ascii=False, indent=2)
#     print(f"\nResults saved to {output_file}")


@app.post("/process-tajweed/")
async def process_tajweed(file: UploadFile = File(...)):
    """Endpoint to process an audio file, perform tajweed classification, and return the result and visualization."""
    # Validate file type
    ...
    try:
        ...
        # Process the audio with tajweed classification
        ...
        plot_buffer = visualize_tajweed_results(result)

        # Prepare the response
        # Convert result to JSON-serializable format
        result_json = json.loads(json.dumps(result, ensure_ascii=False))

        # Create a unique file_id for the processed file
        file_id = f"tajweed_{int(time.time())}_{os.path.basename(temp_file_path)}"
        
        # Store plot buffer for later retrieval
        if "visualization_cache" not in globals():
            globals()["visualization_cache"] = {}
        
        # Convert plot buffer to base64 for direct embedding
        plot_buffer.seek(0)
        plot_base64 = base64.b64encode(plot_buffer.read()).decode('utf-8')
        
        # Store both the buffer and the base64 string
        globals()["visualization_cache"][file_id] = {
            "buffer": plot_buffer,
            "base64": plot_base64,
            "timestamp": time.time()
        }
        
        # Run TajweedAudioSegmenter analysis
        segmenter = TajweedAudioSegmenter(
            audio_file_path=temp_file_path, tajweed_output=result
        )
        analysis = segmenter.analyze_tajweed()
        
        # Return the result with file_id and base64 visualization
        return {
...
async def get_visualization(file_id: str):
    """Return the visualization image for a specific file_id."""
    # Check if we have a visualization cached for this file_id
    if "visualization_cache" not in globals() or file_id not in globals()["visualization_cache"]:
        raise HTTPException(status_code=404, detail="Visualization not found")
    
    # Get the cached visualization
    viz_data = globals()["visualization_cache"][file_id]
    
    # Check if it's expired (older than 1 hour)
    if time.time() - viz_data["timestamp"] > 3600:
        # Remove from cache
        del globals()["visualization_cache"][file_id]
        raise HTTPException(status_code=404, detail="Visualization expired")
